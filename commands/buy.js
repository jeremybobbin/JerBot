...
module.exports.run = async (bot, message, args, storePrices, CollItems) => {
  ...
  if(!args[0]) return console.log("No Args");
  ...
  // console.log(uCoins);
  if(cost > uCoins) return message.channel.send(`You do not have enough coins ${uCoins} coins`);
  ...
  .setColor("#2FE0D7")
  .addField(`${message.author.username}'s Updated Items:`, `
    **Milk amount:** ${umilk}
    **Maid amount:** ${umaid}
    **Food amount:** ${ufood}
    **Geese amount:** ${ugeese}
